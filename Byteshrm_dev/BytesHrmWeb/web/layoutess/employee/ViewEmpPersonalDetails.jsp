<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
         pageEncoding="ISO-8859-1"%>
<%@taglib uri="/struts-tags" prefix="s"%>
<!--[if IE 8]> <html lang="en" class="ie8 no-js"> <![endif]-->
<!--[if IE 9]> <html lang="en" class="ie9 no-js"> <![endif]-->
<!--[if !IE]><!-->
<html lang="en">
    <!--<![endif]-->
    <!-- BEGIN HEAD -->

    <head>
        <meta charset="utf-8" />
        <title>      <jsp:include page="../layout/_inc_page_title.jsp"/></title>

        <%@include file="../layout/_inc_head.jsp" %>

    </head>
    <body class="page-header-fixed page-sidebar-closed-hide-logo page-container-bg-solid page-content-white <%=session.getAttribute("SIDE_BAR_TOGGLE")%>">


        <div class="page-wrapper">
            <%@include file="../layout/_inc_header.jsp" %>       
            <!-- BEGIN CONTAINER -->
            <div class="page-container">
                <%@include file="../layout/_inc_left_menu.jsp" %>
                <!-- BEGIN CONTENT -->
                <div class="page-content-wrapper">
                    <!-- BEGIN CONTENT BODY -->
                    <div class="page-content">
  
                        <s:form  id="ePersonalDetail" name="ePersonalDetail"  action="ePersonalDetail.itb"   cssClass="form-horizontal" method="post" theme="simple" novalidate="novalidate"  >
                            <s:hidden name="employee.empNumber" id="employee.empNumber"></s:hidden>                                                        
                            <s:hidden name="employee.empPicture.empNumber"  value="%{employee.empNumber}" id="employee.empPicture.empNumber"></s:hidden>

                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="tabbable-line boxless tabbable-reversed">
                                            <ul class="nav nav-tabs">
                                                <li class="active">
                                                <s:a href="gPersonalDetail.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Personal Details </s:a>
                                                </li>

                                                <li   >
                                                <s:a href="gContactDetail.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Contact Details </s:a>
                                                </li>
                                                <li  >
                                                <s:a href="gEmergencyContacts.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Emergency Contacts </s:a>
                                                </li>
                                                <li  >
                                                <s:a href="gDependents.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Dependents </s:a>
                                                </li>
                                              <li  >
                                             
                                                           <s:a href="gEmpImmigrations.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Immigration </s:a>

                                              </li>
                                            <li  >
                                             
                                                           <s:a href="gJob.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Job </s:a>

                                              </li>
                                           <li  >

                                                <s:a href="gSalaries.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Salary </s:a>

                                                </li>
                                             <li  >

                                                <s:a href="gReporting.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Report-To </s:a>

                                                </li>

                                   <li  >
                                            
                                              <s:a href="cEmpMemberDetail.itb?empNumber=%{employee.empNumber}"  id="empNumber" name="employee.empNumber"  value="employee.empNumber"> Membership </s:a>

                                            
                                            </li>

                                        </ul>
                                        <div class="tab-content">
                                            <div class="tab-pane active" id="tab_personal_details">
                                                <div class="portlet box blue">
                                                    <div class="portlet-title">
                                                        <div class="caption">
                                                            <i class="fa fa-gift"></i>Personal Details</div>
                                                        <div class="tools">
                                                            <a href="javascript:;" class="collapse"> </a>
                                                            <a href="#portlet-config" data-toggle="modal" class="config"> </a>
                                                            <a href="javascript:;" class="reload"> </a>
                                                            <a href="javascript:;" class="remove"> </a>
                                                        </div>
                                                    </div>


                                                    <div class="portlet-body form">
                                                        <!-- BEGIN FORM--> <div class="form-body">





                                                            <jsp:include page="../layoutcustom/CustomMessage.jsp" />
           
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">First Name <span class="color-red">*</span></label>
                                                                        <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empFirstname" id="empFirstname"   placeholder="First Name"  /> 
                                                                            <span class="help-block"> <s:fielderror   fieldName="empFirstname"/> </span>



                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <!--/span-->
                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Middle Name</label>
                                                                        <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empMiddleName" id="empMiddleName"   placeholder="Middle Name"  /> 
                                                                            <span class="help-block"> <s:fielderror   fieldName="empMiddleName"/> </span>



                                                                        </div>
                                                                    </div>
                                                                </div>

                                                            </div>
                                                            <!--/row-->
                                                            <div class="row">

                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Last Name</label>
                                                                        <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empLastname" id="empLastname"   placeholder="Last Name"  /> 
                                                                            <span class="help-block"> <s:fielderror   fieldName="empLastname"/> </span>



                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>


                                                                
                                                                <!--/span-->


                                                            </div>
                                                            <!--/row-->
                                                            <div class="row">




                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Employee#</label>
                                                                        <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.employeeId" id="employeeId"   placeholder="Employee Number"  /> 
                                                                            <span class="help-block"> <s:fielderror   fieldName="employeeId"/> </span>



                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <!--/span-->

                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Other Id</label>
                                                                        <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empOtherId" id="empOtherId"   placeholder="Other Id"  /> 
                                                                            <span class="help-block"> <s:fielderror   fieldName="empOtherId"/> </span>



                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>


                                                            </div>
                                                            <!--/row-->



                                                            <div class="row">


                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">License Number#</label>
                                                                        <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empDriLiceNum" id="empDriLiceNum"   placeholder="Driver's License Number"  /> 
                                                                            <span class="help-block"> <s:fielderror   fieldName="empDriLiceNum"/> </span>



                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <!--/span-->

                                                                <div class="col-md-6" >
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">License Expiry Date</label>
                                                                        <div class="col-md-9">


                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empDriLiceExpDate" id="mask_date_mmddyyyy"    /> 



                                                                            <span class="help-block">  Like [mm/dd/yyyy]  <s:fielderror   fieldName="empDriLiceExpDate"/> </span>



                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>


                                                            </div>
                 

                                                            <!--/row-->
                                                            <div class="row">

                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Gender</label>
                                                                        <div class="col-md-9">
                                                                            <s:select cssClass="bs-select form-control" data-live-search="true" data-size="8"  name="employee.empGender"  id="empGender"  headerValue="Select Gender "   headerKey=""  list="refDto.genderList"
                                                                                      listKey="code" listValue="name"></s:select>
                                                                            </div>
                                                                        </div>
                                                                    </div>

                                                                    <!--/span-->

                                                                    <div class="col-md-6">
                                                                        <div class="form-group">
                                                                            <label class="control-label col-md-3">Marital Status</label>
                                                                            <div class="col-md-9">
                                                                            <s:select cssClass="bs-select form-control" data-live-search="true" data-size="8"  name="employee.empMaritalStatus"  id="empMaritalStatus"  headerValue="Select Marital Status "   headerKey=""  list="refDto.maritalStatusList"
                                                                                      listKey="code" listValue="name"></s:select>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>
                                                                <!--/row-->

                                                                <!--/row-->
                                                                <div class="row">

                                                                    <div class="col-md-6">
                                                                        <div class="form-group">
                                                                            <label class="control-label col-md-3">Nationality</label>
                                                                            <div class="col-md-9">
                                                                            <s:select cssClass="bs-select form-control" data-live-search="true"  name="employee.nationCode.id"  id="nationCode"   list="refDto.nationalityList"
                                                                                      listKey="id" listValue="name"></s:select>
                                                                            </div>
                                                                        </div>
                                                                    </div>

                                                                    <!--/span-->

                                                                    <div class="col-md-6" >
                                                                        <div class="form-group">
                                                                            <label class="control-label col-md-3">Date of Birth</label>
                                                                            <div class="col-md-9">
                                                                            <s:textfield cssClass="form-control"  autocomplete="off"  
                                                                                         name="employee.empBirthday"     id="mask_date_mmddyyyy1" /> 
                                                                            <span class="help-block"> Like [mm/dd/yyyy] <s:fielderror   fieldName="empBirthday"/> </span>
                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>
                                                                <!--/span-->
                                                            </div>
                                                            <!--/row-->
                                                            <h3 class="form-section"></h3>
                                                            <!--/row-->
                                                            <div class="row">



                                                                <div class="col-md-6" >
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Attachment</label>
                                                                        <div class="col-md-9">
                                                                            <s:file   requiredLabel="false"  cssClass="dropzone dropzone-file-area"  />  
                                                                            <span class="help-block"> <s:fielderror   fieldName="Attachment"/> </span>
                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>



                                                                <!--/span-->

                                                                
                                                            </div>
                                                            <!--/row-->

                                                            <!--/row-->
                                                            <div class="row">

                                                                <div class="col-md-6" >
                                                                    <div class="form-group">
                                                                        <label class="control-label col-md-3">Notes</label>
                                                                        <div class="col-md-9">
                                                                            <s:textarea   cssClass="form-control"  autocomplete="off"  
                                                                                          name="employee.empPicture.epicType" /> 
                                                                            <span class="help-block">
                                                                                <s:fielderror   fieldName="epicType"/> 
                                                                            </span>



                                                                        </div>
                                                                    </div>
                                                                    <!--/span-->
                                                                </div>

             
                                                                <!--/span-->


                                                                <!--/span-->
                                                            </div>
                                                            <!--/row-->
                                                            <div class="form-actions">
                                                                <a href="mEmployee.itb" data-toggle="confirmation" data-original-title="Are you sure ?" class="btn red btn-outline">
                                                                    <i class="fa fa-mail-reply"></i> Cancel </a>
                                                                <a href="gPersonalDetail.itb?empNumber=<s:property value="%{employee.empNumber}" />" data-toggle="confirmation" data-original-title="Are you sure ?" class="btn blue btn-outline">
                                                                    <i class="fa fa-remove"></i> Reset </a>
                                                                <s:submit method="saveEmpPersonalDetail" data-toggle="confirmation" data-original-title="Are you sure ?"  cssClass="btn blue btn-outline"   id="savePersonalInfo" key="Save Personal Info" />
                                                            </div>
                                                            

                                                            <!-- END FORM-->  


                                                        </div>
                                                    </div>

                                                </div>


                                            </div>



                                        </div>

                                    </div>
                                </div>
                            </div>



                        </s:form> 
                    </div>
                    <!-- END CONTENT BODY -->
                </div>
            </div>
        </div>

        <!--  layout/_inc_footer_quick_nav.jsp-->
        <%@include file="../layout/_inc_footer_script.jsp" %>

        <!-- BEGIN PAGE LEVEL PLUGINS -->

        <!-- END PAGE LEVEL PLUGINS -->

        <!-- BEGIN PAGE LEVEL SCRIPTS -->

        <!-- END PAGE LEVEL SCRIPTS -->



    </body>

</html>