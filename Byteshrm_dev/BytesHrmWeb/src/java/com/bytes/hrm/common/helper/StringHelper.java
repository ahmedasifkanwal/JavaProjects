...
public class StringHelper {
    ...
    public static void main(String argv[]) throws ParseException, Exception {
        ...
        Date test =  DateHelper.getFormatString(startDate);
        
        
        SysUserActionService sysUserActionService = new SysUserActionServiceImpl();

        Collection< SysUserAction> sysUserActionList = sysUserActionService.findUserActionByRole("Admin");
        
        //get all Modules.
        Object [] user= sysUserActionList.toArray();
      
         Map<Integer, SysModules> m2 = new HashMap<>();
         List modules=new ArrayList();
        for (Object user1 : user) {
            SysUserAction a = (SysUserAction) user1;
            m2.put(a.getModuleId().getId(), a.getModuleId());
        }
       
      for (Map.Entry<Integer,SysModules> entry : m2.entrySet())  
      {
       modules.add( entry.getValue());
      }
     
        // modules.add( a.getModuleId());
      

        SysUserDto sysUserDto = new SysUserDto();

        sysUserDto.getSysUser().setUsername("zoha");
        sysUserDto.getSysUser().setPassword("saleem");
        sysUserDto.getSysUser().setUserAuthority("ESS");
        sysUserDto.getSysUser().setCreatedBy("saleem");
        ...
        if (new SysUserServiceImpl().create(sysUserDto.getSysUser()) != null) {
        }

        SysUserService service = new SysUserServiceImpl();
        SysUser u = service.findSysUsers("saleem");

        //System.out.print(new SysAuthoritiesServiceImpl().findAllRole().size());
        EmployeeService employeeService = new EmployeeServiceImpl();
        Map<String, Object> m = new HashMap<String, Object>();
        m.put("o.employeeId", "EMP0012");
        Collection<  Employee> e = employeeService.search(m);
        int j = e.size();
        Employee empL = (Employee) e.toArray()[0];

        Employee empL1 = new EmployeeServiceImpl().getEmployee("EMP0012");
